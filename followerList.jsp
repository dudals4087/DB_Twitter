...
<%
    ...
    try {
        // [수정] ORDER BY f.created_at -> ORDER BY u.user_id (날짜 컬럼 없으므로 아이디순 정렬)
        String sql = "SELECT u.user_id, u.status_message, u.paid, u.profile_img " +
                     "FROM followings f " +
                     "JOIN users u ON u.user_id = f.follower_id " + 
                     "WHERE f.user_id = ? " + 
                     "ORDER BY u.user_id ASC"; // 여기를 수정했습니다!
...
<head>
    <meta charset="UTF-8">
    <title><%= profileUser %>의 팔로워 / TWITTER_DB4</title>
...
<div class="app-shell">
    <header class="app-header">
        <div class="app-header-left">
            ...
            <div class="app-logo-sub">팔로워</div>
        </div>
    ...
    <div class="main-layout">
        <section class="column-center">
            <div class="card">
                <div class="section-title"><%= profileUser %>님을 따르는 사람들</div>
            </div>

            <div class="card">
                <% if (userList.isEmpty()) { %>
                    <div class="helper-text" style="padding:20px; text-align:center;">팔로워가 없습니다.</div>
                <% } else { 
